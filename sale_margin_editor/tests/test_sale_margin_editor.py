#  Copyright 2020 TechnoLibre
#  License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl)
from datetime import datetime
from odoo.tests import common, tagged


class TestSaleMarginEditor(common.SavepointCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_sale_margin_update_global(self):
        print("MATHBEN TEST TEST TEST TEST TEST +++++++++++++++++++++")
        ...
        self.assertEqual(order_line.purchase_price, 10.0)
        self.assertEqual(order_line.margin_percent, 50.0)
    ...
    # valider les switchs de pourcentage

    # changer % global ( tester -100, -50, 0, 50, 100, 300, -300)
    ...
    # ligne % global ( avec lock service et product sur la moitié des items)

    # ligne % product ( avec lock service et product sur la moitié des items)

    # ligne % service ( avec lock service et product sur la moitié des items)

    # ligne cost ( comportement à 0, et autre)

    # Ligne unit_price ( comportemet à 0 et autre)

    # ligne Margin$ ( comportement à 0, négatif et positif)

    # ligne Margin% ( tester -100, -50, 0, 50, 100, 300, -300)

    # ligne quantité ( 0, 40, vérifier tous les autre champs)
